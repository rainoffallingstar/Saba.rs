#!/usr/bin/env bash
# Builds a Linux AppImage of the native Saba.rs client.
set -euo pipefail
...
cd "$ROOT_DIR"
cargo build --release --locked -p sabaki-gpui --bin "$APP_NAME"
...
rm -rf "$APPDIR"
mkdir -p "$APPDIR/usr/bin" "$APPDIR/usr/share/licenses/saba-rs" "$APPDIR/usr/share/mime/packages" "$APPDIR/usr/share/icons/hicolor/scalable/apps"
cp "$BINARY" "$APPDIR/usr/bin/$APP_NAME"
cp "$ROOT_DIR/LICENSE.md" "$APPDIR/usr/share/licenses/saba-rs/LICENSE.md"
cp "$ROOT_DIR/flatpak/dev.sabars.app.xml" "$APPDIR/usr/share/mime/packages/dev.sabars.app.xml"
...
mkdir -p "$OUTPUT_DIR"
OUTPUT="$OUTPUT_DIR/saba-rs-v$VERSION-linux-x86_64.AppImage"
