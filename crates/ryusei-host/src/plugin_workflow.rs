...
use ryusei_plugin_runtime::{PluginError, PluginPermission, PluginRecord};
...
impl PluginStore {
    ...
    pub fn restore(
        ...
        for record in &mut records {
            let is_builtin = record.manifest.id == "org.ryusei.katago-setup-hub"
                || record.manifest.id == "org.ryusei.fox-kifu-sync"
                || record.manifest.id == "org.ryusei.position-checker"
                || record.manifest.id == "org.ryusei.sgf-exporter";
...
mod tests {
    ...
    use ryusei_plugin_runtime::{PluginError, PluginManifest, PluginPermission, PluginRecord};
    use std::{
        cell::RefCell,
    ...
    fn sample_manifest(id: &str, name: &str) -> PluginManifest {
        PluginManifest {
            ...
            api_version: 1,
            runtime: ryusei_plugin_runtime::PluginRuntime::Declarative,
    ...
    fn fresh_plugin_root(test_name: &str) -> PathBuf {
        let temp_dir =
            std::env::temp_dir().join(format!("ryusei-host-{test_name}-{}", std::process::id()));
