...
use ryusei_domain_core::gtp::GtpProcessSupervisor;
...
fn start_real_katago() -> Option<GtpProcessSupervisor> {
    ...
    let Some(engine) = ryusei_host::find_katago_executable(None) else {
...
fn full_session_handshake_and_replay_with_real_katago() {
    ...
    use ryusei_host::{EngineController, EngineRecord, ProcessGtpTransport};
