...
> `scripts/installer.nsi`、`flatpak/dev.ryusei.app.yml`。
> 签名/公证（需证书）待后续。
...
3. ✅ Linux AppImage 构建（linuxdeploy + appimagetool，含依赖收集）；Flatpak
   manifest（`flatpak/dev.ryusei.app.yml`，Freedesktop 24.08 + rust-stable
   extension）已入 release 工作流。Linux/Flatpak desktop entry 和 MIME 声明覆盖
   SGF/NGF/GIB/UGF，仍需在已安装产物中验证双击打开。
