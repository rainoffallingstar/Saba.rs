...
pub fn apply_process_info(entry: &mut PluginPanelEntry, info: &ryusei_host::PluginProcessInfo) {
    if !entry.native_runtime {
        return;
    }
    let status = match info.status {
        ryusei_host::PluginProcessStatus::Running => {
            format!("running ({} restarts)", info.restart_count)
        }
        ryusei_host::PluginProcessStatus::Crashed => {
            format!("crashed ({} crashes)", info.crash_count)
        }
        ryusei_host::PluginProcessStatus::Disabled => {
            format!("auto-disabled after {} crashes", info.crash_count)
        }
        ryusei_host::PluginProcessStatus::Stopped => "stopped".to_owned(),
    };
...
mod tests {
    ...
    use ryusei_plugin_runtime::{PluginManifest, PluginRecord};
    ...
    fn record_entry_reflects_enabled_state_and_granted_permissions() {
        ...
        let record = PluginRecord {
            ...
            granted_permissions: BTreeSet::from([
                ryusei_plugin_runtime::PluginPermission::GameRead,
            ]),
    ...
    fn native_plugins_report_runtime_and_authorization_state() {
        ...
        manifest.runtime = ryusei_plugin_runtime::PluginRuntime::Native;
        ...
        let record = PluginRecord {
            ...
            granted_permissions: BTreeSet::from([
                ryusei_plugin_runtime::PluginPermission::GameRead,
                ryusei_plugin_runtime::PluginPermission::UiPanel,
            ]),
            native_execution_authorized: false,
        };
    ...
    fn process_info_overlays_running_and_crashed_state() {
        use ryusei_host::{PluginProcessInfo, PluginProcessStatus};
        let mut manifest = sample_manifest();
        manifest.runtime = ryusei_plugin_runtime::PluginRuntime::Native;
        manifest.entrypoint = Some("bin/plugin".to_owned());
        let record = PluginRecord {
