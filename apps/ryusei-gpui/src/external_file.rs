...
//!
//! The detection state machine lives in `ryusei_host::ExternalFileStore`; this
...
pub fn check_external_file(
    ...
    host: &mut ryusei_host::HostApplication,
...
fn detect_encoding_or_default(content: &str) -> SourceEncoding {
    ryusei_host::detect_sgf_encoding(content.as_bytes())
        .ok()
        .flatten()
...
impl ryusei_host::HostEventSink for RecordingEventSink {
...
mod tests {
    ...
    fn fresh_test_directory(test_name: &str) -> PathBuf {
        ...
        let directory = std::env::temp_dir().join(format!(
            "ryusei-gpui-external-{test_name}-{}-{counter}",
            std::process::id()
        ));
    ...
    fn reader_returns_decoded_content_for_legacy_encodings() {
        ...
        let bytes = ryusei_host::encode_sgf(source_sgf, SourceEncoding::ShiftJis)
    ...
    fn dirty_documents_keep_the_change_status_without_reloading() {
        ...
        host.play_move(
            ...
            Some(ryusei_domain_core::Vertex { column: 3, row: 15 }),
            &mut events,
        )
