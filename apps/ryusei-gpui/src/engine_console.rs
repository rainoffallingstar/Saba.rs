use ryusei_domain_core::gtp::{GtpError, GtpResponse, parse_response};

/// A minimal GTP engine port. The real process transport already lives in
/// `ryusei_domain_core::GtpProcessSupervisor`; the shell console uses an
...
impl ryusei_host::GtpTransport for MockGtpEngine {
...
pub fn parse_engine_spec(spec: &str) -> Result<ryusei_host::EngineRecord, String> {
    ...
    let mut record = ryusei_host::EngineRecord::new(name, path, args);
    ...
    ryusei_host::validate_engine_record(&value).map_err(|error| error.to_string())?;
    Ok(record)
}
...
impl EngineRoleAssignments {
    pub fn from_settings(settings: &ryusei_host::SettingsStore) -> Self {
        Self {
            analysis: settings
...
pub fn best_analysis_entry(
    entries: &[ryusei_host::AnalysisEntry],
) -> Option<&ryusei_host::AnalysisEntry> {
    entries
        .iter()
...
pub fn merge_analysis_entries(
    existing: &[ryusei_host::AnalysisEntry],
    new: Vec<ryusei_host::AnalysisEntry>,
) -> Vec<ryusei_host::AnalysisEntry> {
    use std::collections::BTreeMap;
    let mut by_vertex: BTreeMap<String, ryusei_host::AnalysisEntry> = existing
        .iter()
...
pub fn parse_engine_arguments(args: &str) -> Vec<String> {
    ryusei_host::parse_engine_arguments(args)
}
...
#[allow(dead_code)]
pub fn parse_stream_line(_command: &str, line: &str) -> Option<ryusei_host::AnalysisEntry> {
...
pub fn parse_stream_entries(_command: &str, line: &str) -> Vec<ryusei_host::AnalysisEntry> {
    if line.trim_start().starts_with('{') {
        ryusei_host::parse_kata_analysis_line(line)
            .into_iter()
            .collect()
    } else {
        ryusei_host::parse_lz_analysis_entries(line)
    }
}
...
mod tests {
    ...
    fn parses_quoted_and_unquoted_engine_arguments() {
        let args = super::parse_engine_arguments(
            r#"gtp -model "/Users/foo/Library/Application Support/ryusei/models/b10c.bin.gz" -config "/path with spaces/cfg.cfg" --threads 8"#,
        );
        assert_eq!(
            args,
            vec![
                "gtp",
                "-model",
                "/Users/foo/Library/Application Support/ryusei/models/b10c.bin.gz",
                "-config",
    ...
    fn host_session_drives_the_mock_engine_through_the_lifecycle() {
        use ryusei_host::{EngineRecord, EngineSession, EngineSessionState};
    ...
    fn mock_engine_answers_analysis_requests() {
        ...
        let entries = ryusei_host::parse_analysis_response("lz-analyze", &response.content);
    ...
    fn best_analysis_move_prefers_the_most_visited_candidate() {
        ...
        let empty: Vec<ryusei_host::AnalysisEntry> = Vec::new();
        assert_eq!(best_analysis_move(&empty, 19), None);
        assert_eq!(
            best_analysis_winrate(&empty, ryusei_domain_core::Color::Black),
            0.0
        );
    }
