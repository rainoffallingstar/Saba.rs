...
fn select_config_directory(config_root: PathBuf) -> PathBuf {
    let primary = config_root.join("ryusei");
    let legacy = config_root.join("ryusei-gpui");
...
/// Resolves the config directory from `RYUSEI_CONFIG_DIR` when set. New
/// installations use `$HOME/.config/ryusei`; an existing `ryusei-gpui`
...
pub fn current_user_config_directory() -> Result<PathBuf, String> {
    let config_directory = match std::env::var("RYUSEI_CONFIG_DIR") {
        Ok(path) => PathBuf::from(path),
        Err(_) => {
            let home = std::env::var("HOME")
                .or_else(|_| std::env::var("USERPROFILE"))
                .map_err(|_| "neither RYUSEI_CONFIG_DIR, HOME nor USERPROFILE is set".to_owned())?;
...
pub fn seed_builtin_plugins(install_root: &std::path::Path) {
    ...
    let _ = std::fs::write(
        katago_hub_dir.join("ryusei-plugin.json"),
        include_str!("../../../examples/plugins/katago-setup-hub/ryusei-plugin.json"),
    );

    let fox_kifu_dir = install_root.join("fox-kifu-sync");
    let _ = std::fs::create_dir_all(&fox_kifu_dir);
    let _ = std::fs::write(
        fox_kifu_dir.join("ryusei-plugin.json"),
        include_str!("../../../examples/plugins/fox-kifu-sync/ryusei-plugin.json"),
    );

    let pos_checker_dir = install_root.join("position-checker");
    let _ = std::fs::create_dir_all(&pos_checker_dir);
    let _ = std::fs::write(
        pos_checker_dir.join("ryusei-plugin.json"),
        include_str!("../../../examples/plugins/position-checker/ryusei-plugin.json"),
    );

    let sgf_exp_dir = install_root.join("sgf-exporter");
    let _ = std::fs::create_dir_all(&sgf_exp_dir);
    let _ = std::fs::write(
        sgf_exp_dir.join("ryusei-plugin.json"),
        include_str!("../../../examples/plugins/sgf-exporter/ryusei-plugin.json"),
    );
}
...
impl HostPersistence for NativeHostPersistence {
    fn load_autosave(&self) -> AutosaveStore {
        ...
        let Ok(recovery) =
            serde_json::from_slice::<ryusei_host::autosave::PersistedAutosave>(&content)
        else {
            return AutosaveStore::default();
        };
        if recovery.schema_version != ryusei_host::autosave::AUTOSAVE_SCHEMA_VERSION
    ...
    fn load_recent_files(&self) -> Result<RecentFilesStore, String> {
        ...
        let persisted = ryusei_host::recent_files::persisted_recent_files_from_bytes(&content)
            .map_err(|error| format!("could not parse recent files: {error}"))?;
        if persisted.schema_version != ryusei_host::recent_files::RECENT_FILES_SCHEMA_VERSION {
            return Err(format!(
    ...
    fn persist_recent_files(&self, store: &RecentFilesStore) -> Result<(), String> {
        let content =
            ryusei_host::recent_files::persisted_recent_files_to_bytes(&store.to_persisted())
...
mod tests {
    ...
    use ryusei_plugin_runtime::PluginPermission;
    use std::path::PathBuf;

    fn fresh_config_directory(test_name: &str) -> PathBuf {
        let directory = std::env::temp_dir().join(format!("ryusei-gpui-{test_name}"));
    ...
    fn dirty_document_captures_a_recovery_and_clean_save_clears_it() {
        ...
        host.play_move(
            ...
            Some(ryusei_domain_core::Vertex { column: 3, row: 3 }),
            &mut events,
        )
        .unwrap();
    ...
    fn plugin_registry_persists_through_the_boundary() {
        ...
        let mut store = PluginStore::from_records(vec![ryusei_plugin_runtime::PluginRecord {
    ...
    fn config_directory_selection_preserves_legacy_data_until_migrated() {
        ...
        assert_eq!(select_config_directory(root.clone()), root.join("ryusei"));

        std::fs::create_dir_all(root.join("ryusei-gpui")).expect("legacy config exists");
        assert_eq!(
            select_config_directory(root.clone()),
            root.join("ryusei-gpui"),
            "an existing legacy directory is used until a new primary directory exists"
        );

        std::fs::create_dir_all(root.join("ryusei")).expect("new config exists");
        assert_eq!(select_config_directory(root.clone()), root.join("ryusei"));
    ...
    fn native_plugin_persistence_round_trips_registry_states() {
        ...
        std::fs::write(
            install_path.join("ryusei-plugin.json"),
            r#"{
                "schemaVersion": 1,
                "id": "org.example.opening-trainer",
                "name": "Opening Trainer",
