...
pub fn window_bounds_from_settings(settings: &sabaki_host::SettingsStore) -> Option<(f64, f64)> {
    let width = settings
        .get("window.width")
...
pub fn window_maximized_from_settings(settings: &sabaki_host::SettingsStore) -> bool {
    settings
...
/// shared key table owned by `sabaki-host` (mirroring the Electron schema).
#[allow(dead_code)]
...
mod tests {
    ...
    fn window_maximized_falls_back_when_the_load_drops_invalid_values() {
        ...
        impl sabaki_host::SettingsPersistence for RawSettingsPersistence {
        ...
        let loaded = sabaki_host::load_settings_store(&persistence).expect("raw settings load");
